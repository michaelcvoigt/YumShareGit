...
namespace MalbersAnimations.Selector
{
    ...
    public partial class SelectorController
    {
        ...
        protected void Animate_to_Next_Item()
        {
            ...
            {
                Debug.Log("Animate_to_Next_Item");
                // if (RestoreTransformAnimationC != null) StopCoroutine(RestoreTransformAnimationC);
        ...
        protected virtual void SelectionAction()
        {
            ...
            if (MainInputDown)                                          //The moment the mouse/touch start the click/touch
            {
                ...
                if (CInertia != null)
                {
                    ...
                    CheckForWorldRotation();
                    if (isInInertia) SetNextItemRadial(InertiaLastDrag);
                }
            }
        ...
        private void SetNextItemRadial(float distance)
        {
            int Next = Mathf.RoundToInt(distance / S_Editor.Angle);
        ...
        IEnumerator C_InertiaLineal(float LastSpeed)
        {
            ...
            IndexSelected = Mathf.RoundToInt(Mathf.Abs(InertiaLastDrag) / distance) % Items.Count;
